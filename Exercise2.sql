...
SELECT nombre AS NOMBRE,SUM(sal_emp + comision_emp + 500) AS PAGO_TOTAL FROM empleados WHERE id_depto = 3000 GROUP BY sal_emp ORDER BY nombre ASC;
...
SELECT sal_emp AS Salario, comision_emp AS Comision, SUM(sal_emp+comision_emp) AS Salario_Total , nombre FROM empleados WHERE comision_emp > 1000 GROUP BY sal_emp, comision_emp, nombre;
# 14. Obtener un listado similar al anterior, pero de aquellos empleados que NO tienen comisión
SELECT sal_emp AS Salario, comision_emp AS Comision, SUM(sal_emp+comision_emp) AS Salario_Total , nombre FROM empleados WHERE comision_emp = 0  GROUP BY sal_emp, comision_emp, nombre;
# 15. Obtener la lista de los empleados que ganan una comisión superior a su sueldo
SELECT nombre AS Empleados_comision_mayor_a_sueldo FROM empleados WHERE comision_emp > sal_emp;
# 16. Listar los empleados cuya comisión es menor o igual que el 30% de su sueldo.
SELECT nombre FROM empleados WHERE comision_emp <= (sal_emp * 0.30);
# 17. Hallar los empleados cuyo nombre no contiene la cadena “MA”
SELECT nombre FROM empleados WHERE nombre NOT LIKE 'MA%' OR '%MA';
...
SELECT nombre,MAX(sal_emp) AS Salario_Mas_Alto FROM empleados ;
...
SELECT MAX(sal_emp)AS Salario_Alto, MIN(sal_emp) AS Salario_Bajo, MAX(sal_emp)-MIN(sal_emp) AS Diferencia FROM empleados;
# 23. Hallar el salario promedio por departamento
SELECT AVG(sal_emp) AS Promedio_Salario FROM empleados GROUP BY id_depto;
...
SELECT cod_jefe, nombre, COUNT(id_emp) As 'Cantidad de empleados' FROM empleados WHERE cargo_emp LIKE "Jefe%" HAVING COUNT(id_emp)>1;
